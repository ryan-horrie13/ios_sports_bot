...
def get_url(user_message_array, count):
    if 'nba' in user_message_array[1]:
        if len(user_message_array[4]) <= 5:
            url = 'https://www.basketball-reference.com/players/' + user_message_array[4][0] + '/' + user_message_array[
                4] + user_message_array[3][0] + user_message_array[3][1] + '0' + str(count) + '.html'
        if len(user_message_array[4]) > 5:
            url = 'https://www.basketball-reference.com/players/' + user_message_array[4][0] + '/' + \
                  user_message_array[4][0] + user_message_array[4][1] + user_message_array[4][2] + \
                  user_message_array[4][3] + user_message_array[4][4] + user_message_array[3][0] + \
                  user_message_array[3][1] + '0' + str(count) + '.html'
        return url
    if 'mlb' in user_message_array[1]:
        if len(user_message_array[4]) <= 5:
            url = 'https://www.baseball-reference.com/players/' + user_message_array[4][0] + '/' + user_message_array[
                4] + user_message_array[3][0] + user_message_array[3][1] + '0' + str(count) + '.shtml'
        if len(user_message_array[4]) > 5:
            url = 'https://www.baseball-reference.com/players/' + user_message_array[4][0] + '/' + \
                  user_message_array[4][0] + user_message_array[4][1] + user_message_array[4][2] + \
                  user_message_array[4][3] + user_message_array[4][4] + user_message_array[3][0] + \
                  user_message_array[3][1] + '0' + str(count) + '.shtml'
        return url
    if 'ncaa-basketball' in user_message_array[1]:
        url = 'https://www.sports-reference.com/cbb/players/' + user_message_array[3] + '-' + user_message_array[
            4] + '-' + str(count) + '.html'
        return url
    if 'ncaaf' in user_message_array[1]:
        url = 'https://www.sports-reference.com/cfb/players/' + user_message_array[3] + '-' + user_message_array[
            4] + '-' + str(count) + '.html'
        return url
    if 'nfl' in user_message_array[1]:
        ...
        if len(user_message_array[4]) <= 4:
            url = 'https://www.pro-football-reference.com/players/' + last_name_capital[0] + '/' + last_name_capital + \
                  first_name_capital[0] + first_name_capital[1] + '0' + str(count) + '.htm'
        if len(user_message_array[4]) > 4:
            url = 'https://www.pro-football-reference.com/players/' + last_name_capital[0] + '/' + last_name_capital[
                0] + last_name_capital[1] + last_name_capital[2] + last_name_capital[3] + first_name_capital[0] + \
                  first_name_capital[1] + '0' + str(count) + '.htm'
        return url
...
def get_stats(url, sport):
    ...
    if sport == 'ncaa-basketball':
        stats = soup.findAll(attrs={
            'data-stat': ['season', 'g', 'mp_per_g', 'fg_pct', 'fg3_pct', 'ft_pct', 'trb_per_g', 'ast_per_g',
                          'stl_per_g', 'blk_per_g', 'tov_per_g', 'pf_per_g', 'pts_per_g']})  # 13
    ...
    if sport == 'mlb-pitching':
        stats = soup.findAll(attrs={
            'data-stat': ['year_ID', 'player_stats_summary_explain', 'W', 'L', 'earned_run_avg', 'CG', 'SHO', 'IP', 'H',
                          'ER', 'HR', 'BB', 'SO', 'strikeouts_per_nine']})  # 13
        teams = soup.findAll(attrs={'data-stat': 'team_ID'})
    if sport == 'mlb-batting':
        stats = soup.findAll(attrs={
            'data-stat': ['year_ID', 'player_stats_summary_explain', 'G', 'R', 'H', '2B', '3B', 'HR', 'RBI', 'SB', 'BB',
                          'batting_avg', 'onbase_perc', 'slugging_perc']})
        teams = soup.findAll(attrs={'data-stat': 'team_ID'})
    if sport == 'nfl-skill':
        stats = soup.findAll(attrs={
            'data-stat': ['year_id', 'g', 'rush_yds', 'rush_td', 'rush_yds_per_att', 'rush_yds_per_g', 'rec', 'rec_yds',
                          'rec_td', 'yds_from_scrimmage', 'rush_receive_td', 'fumbles']})  # 13
        teams = soup.findAll(attrs={'data-stat': 'team'})
        num_of_stats = 12
    if sport == 'nfl-passing':
        stats = soup.findAll(attrs={
            'data-stat': ['year_id', 'g', 'qb_rec', 'pass_cmp', 'pass_att', 'pass_cmp_perc', 'pass_yds', 'pass_td',
                          'pass_int', 'pass_rating', 'qbr', 'pass_sacked', 'gwd']})  # 13
        teams = soup.findAll(attrs={'data-stat': 'team'})
    if sport == 'ncaaf-passing':
        stats = soup.findAll(attrs={
            'data-stat': ['year_id', 'g', 'pass_cmp', 'pass_att', 'pass_cmp_pct', 'pass_yds', 'pass_td', 'pass_int',
                          'pass_rating']})
        teams = soup.findAll(attrs={'data-stat': 'school_name'})
        num_of_stats = 9
    if sport == 'ncaaf-skill':
        stats = soup.findAll(attrs={
            'data-stat': ['year_id', 'g', 'rush_yds', 'rush_td', 'rec', 'rec_yds', 'rec_td', 'scrim_yds', 'scrim_td']})
        teams = soup.findAll(attrs={'data-stat': 'school_name'})
        num_of_stats = 9
        ...
        if season_or_career in season_stats[i][0].upper():
            if sport == 'mlb-pitching':
                career_stats = '{0} Stats\nTeams:{13}\n{1} Wins\n{2} Losses\n{3} ERA\n{4} CG\n{5} Shut Outs\n{6} IP\n{7} Hits\n{8} Earned Runs\n{9} Home Runs\n{10} Walks\n{11} Strike Outs\n{12} K/9'.format(
                    season_stats[i][0], season_stats[i][1], season_stats[i][2], season_stats[i][3], season_stats[i][4],
                    season_stats[i][5], season_stats[i][6], season_stats[i][7], season_stats[i][8], season_stats[i][9],
                    season_stats[i][10], season_stats[i][11], season_stats[i][12], player_teams)
            if sport == 'mlb-batting':
                career_stats = '{0} Stats\nTeams:{13}\n{1} Games\n{2} Runs\n{3} Hits\n{4} Doubles\n{5} Triples\n{6} Home Runs\n{7} RBIs\n{8} Steals\n{9} Walks\n{10} AVG\n{11} OBP\n{12} SLUG'.format(
                    season_stats[i][0], season_stats[i][1], season_stats[i][2], season_stats[i][3], season_stats[i][4],
                    season_stats[i][5], season_stats[i][6], season_stats[i][7], season_stats[i][8], season_stats[i][9],
            ...
            if sport == 'nfl-skill':
                if season_stats[0][2] == 'Yds':
                    career_stats = '{0} Stats\nTeams: {12}\n{1} Games\n{2} Rushing Yards\n{3} Rushing TDs\n{4} YPA\n{5} YPG\n{6} Receptions\n{7} Receiving Yards\n{8} Receiving TDs\n{9} Scrammage Yards\n{10} Total TDs\n{11} Fumbles'.format(
                        season_stats[i][0], season_stats[i][1], season_stats[i][2], season_stats[i][3],
                        season_stats[i][4], season_stats[i][5], season_stats[i][6], season_stats[i][7],
                        season_stats[i][8], season_stats[i][9], season_stats[i][10], season_stats[i][11], player_teams)
            ...
            if sport == 'ncaaf-passing':
                career_stats = '{0} Stats\n Teams: {9}\n{1} Games\n{2} Completions\n{3} Attempts\n{4} CMP%\n{5} Pass Yards\n{6} TDs\n{7} INTs\n{8} Rating'.format(
                    season_stats[i][0], season_stats[i][1], season_stats[i][2], season_stats[i][3], season_stats[i][4],
                    season_stats[i][5], season_stats[i][6], season_stats[i][7], season_stats[i][8], player_teams)
            if sport == 'ncaaf-skill':
                if season_stats[0][2] == 'Yds':
                    career_stats = '{0} Stats\nTeams: {9}\n{1} Games\n{2} Rush Yards\n{3} Rush TDs\n{4} Receptions\n{5} Receving Yards\n{6} Receiving TDs\n{7} Scrimmage Yards\n{8} Total TDs'.format(
                        season_stats[i][0], season_stats[i][1], season_stats[i][2], season_stats[i][3],
                        season_stats[i][4], season_stats[i][5], season_stats[i][6], season_stats[i][7],
                        season_stats[i][8], player_teams)
                if season_stats[0][2] == 'Rec':
                    career_stats = '{0} Stats\nTeams: {9}\n{1} Games\n{2} Receptions\n{3} Receiving Yards\n{4} Receiving TDs\n{5} Rushing Yards\n{6} Rush TDs\n{7} Scrimmage Yards\n{8} Total TDs'.format(
                        season_stats[i][0], season_stats[i][1], season_stats[i][2], season_stats[i][3],
                        season_stats[i][4], season_stats[i][5], season_stats[i][6], season_stats[i][7],
